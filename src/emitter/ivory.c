...
ivory( emitter_t            *              e,      // Actual emitter
       ...
       mt_rng_t             *              rng ) { // Random number generator
  ...
  for( n=0; n<e->n_component; n++ ) {
     ...
     // - based on ivory law (proportional to a child langmuir law
     //   with a threshold)
...
#    define EMIT_PARTICLES(X,Y,Z,dir) BEGIN_PRIMITIVE {                                \
       if( e->sp->q_m * (dir fi[i].e##X) > 0 ) {                                       \
         ...
         if ( fabs(fi[i].e##X) < args->thresh_e_norm ) m=0;                              \
         /* This face can emit ... compute the charge of the emitted particles */      \
         qp = g->eps0*g->d##Y*g->d##Z*g->dt*                                           \
            sqrt((1./6.)*fabs(e->sp->q_m*fi[i].e##X*fi[i].e##X*fi[i].e##X)/g->d##X)/   \
            (float)m;                                                                  \
         if( e->sp->q_m<0 ) qp = -qp;                                                  \
