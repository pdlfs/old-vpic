/*
   Utility to combine data files for making fig 9 from Wilks et al
   PoP 2001.

   Usage:  fig9  [num processors] [frame] [a or b for ascii/binary write]
...
int main( int argc, char *argv[] ) {
  ...
  **************************************************************************/
  ...
  foreach_file {
    sprintf(fname, "fig9_electron.%d", ifile);
    if ( !(fparr[ifile]=fopen(fname, "rb")) ) ERROR(("Cannot open phase space files.\n"));
    /* First element in each file is nux */
    fread( &nux, sizeof(int), 1, fparr[ifile] );
  }

  /* For simplicity, take nz = number of processors. */
  /* (This is coarse, but should be sufficient for making comparisons). */
  nz=nproc;

  printf("Total nux, nz points in source data: %d %d\n", nux, nz);
  printf("Frame requested: %d\n", nframe);
  data_vec  = (double *)emalloc((size_t)(nux)*sizeof(*data_vec));
  odata_vec = (double *)emalloc((size_t)(nux*nz)*sizeof(*odata_vec));

  /* Prepare to write movie data file. */
  sprintf(ofname, "fig9.%s", SUFFIX(write_binary));
  if ( !(ofp = fopen( ofname, WRITE_MODE(write_binary) )) ) ERROR(("Cannot open file: %s\n", ofname));
  {
    ...
    for ( iux=0; iux<nux; ++iux )
      for ( iz=0; iz<nz; ++iz )
        odata_vec[iux*nz+iz]=0;
    foreach_file {
      fseek( fparr[ifile], 2*sizeof(int)+nframe*nux*sizeof(double), SEEK_SET );
      fread( data_vec, sizeof(double), nux, fparr[ifile] );
      for ( iux=0; iux<nux; ++iux ) {
        iz = ifile;
	odata_vec[iux*nz+iz]+=data_vec[iux*nz+iz];
      }
    }
  }

  /* Write odata_vec into output stream */
  WRITE_VEC(ofp, odata_vec, nux*nz, sizeof(double), write_binary);
  printf("done.\n");
  fclose(ofp);
