...
begin_initialization {
 ...
 double eps0 = 1;         // Permittivity of space
  ...
  double taui    = 200;      // simulation wci's to run
  double amp = 0.3; // amplitude of injected wave relative to Bo

  double quota   = 1.0;   // run quota in hours
  ...
  double nx = 128;    //  Can probably stretch grid in parallel direction
  double ny = 128;       //  Need to resolve rho in perp direction
  double nz = 128;     //  Need to resolve rho in perp direction
  ...
  int interval = int(10/(wci*dt)); 
  int fields_interval = interval;
  int ehydro_interval = interval;
  int Hhydro_interval = interval;
  int eparticle_interval = 200*interval;
  int Hparticle_interval = 200*interval;
