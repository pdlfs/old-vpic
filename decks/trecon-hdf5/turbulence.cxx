////////////////////////////////////////////////////////////////////////
//
//  Uniform plasma with imposed initial waves
// 
//
///////////////////////////////////////////////////////////////////////
...
//  Variables for new output format
  ...
  int dis_iter;        // : iteration count. 0 means we are not avraging at the moment
...
begin_initialization {
 ...
 double eps0 = 1;         // Permittivity of space
  ...
  double vthe    = 0.05;     //  Electron thermal speed over c
  double wpe_wce = 1.0;      // electron plasma freq / electron cyclotron freq
  double taui    = 4000;      // simulation wci's to run
  double amp = 0.3; // amplitude of injected wave relative to Bo

  double quota   = 3.5;   // run quota in hours
  ...
  //double Lx    = 10*(2.0*pi*di); // size of box in x dimension
  double Lz    = 400.0*di;
  double Lx    = 100.0*di;
  double Ly    = 100.0*di;
  //double Ly    = 0.06*di;     // size of box in y dimension
  //double Lz    = Lx*amp; // size of box in z dimension

  double topology_x = 8;  // Number of domains in x, y, and z
  double topology_y = 16; 
  double topology_z = 16;  

  double nx = 512;    //  Can probably stretch grid in parallel direction
  double ny = 512;       //  Need to resolve rho in perp direction
  double nz = 512;     //  Need to resolve rho in perp direction
  ...
  double Va0 = b0/sqrt(1.0 + mi_me);  // Non-relativistic Alfven speed
  double Va = Va0/sqrt(1.0 + Va0*Va0);  // Relativistic phase velocity
  ...
  int restart_interval = int(20000/(wci*dt));
  //int restart_interval = 3000;
  int energies_interval = int(10/(wci*dt));
  int interval = int(100/(wci*dt)); 
  int fields_interval = interval;
  int ehydro_interval = interval;
  int Hhydro_interval = interval;
  int eparticle_interval = 2*interval;
  int Hparticle_interval = 2*interval;
...
# define RANK_TO_INDEX(rank,ix,iy,iz) BEGIN_PRIMITIVE {                   \
  ...
  define_periodic_grid(  -0.5*Lx, -0.5*Ly, 0,    // Low corner
 			  0.5*Lx, 0.5*Ly, Lz,     // High corner
                          nx, ny, nz,             // Resolution
  ...
  species_t *electron = define_species("electron",-ec/me,1.8*Ne/nproc(),-1,electron_sort_interval,0);
  species_t *ion = define_species("ion",ec/mi,1.8*Ne/nproc(),-1,ion_sort_interval,0);
...
//  Finalize Field Advance
  ...
  if (rank() == 0 ) {
    ...
    fp_info.print( " Va0/c =%e\n", Va0/c );
    fp_info.print( " Va/c =%e\n", Va/c );
    fp_info.print( " ***************************\n");
    fp_info.close();
  ...
  double kx0 = 2.0*pi/Lx;
  double ky0 = 2.0*pi/Ly;
  double kz0 = 2.0*pi/Lz;
...
#define DEY_1(l,m,phi) -amp*(l/abs(l))*Va*b0*cos(l*kz0*z + m*ky0*y + phi)
// These give velocity & current consistent with Alfven wave
#define DUX_1(l,m,phi) -amp*(l/abs(l))*Va*cos(l*kz0*z + m*ky0*y + phi)
...
#define DEX_2(l,m,phi) amp*(l/abs(l))*Va*b0*cos(l*kz0*z + m*kx0*x + phi)
// These give velocity & current consistent with Alfven wave
#define DUY_2(l,m,phi) -amp*(l/abs(l))*Va*cos(l*kz0*z + m*kx0*x + phi)
...
#define JZ_PERT_2 DJZ_2(-1,1,0.4) + DJZ_2(-1,-2,2.56) + DJZ_2(2,-3,4.19)
  ...
  set_region_field( everywhere, EX_PERT_2, EY_PERT_1, 0.0, BX_PERT_1, BY_PERT_2, b0);
  ...
  repeat ( Ne/nproc() ) {
    double x, y, z, ux, uy, uz, vx, vy, vz, gamma, v2 ;
   ...
   //  To avoid confusion - first pick velocity of particle

   vx = maxwellian_rand(vthe) + UX_PERT_1 - JX_PERT_2*0.5 ;
   vy = maxwellian_rand(vthe) - JY_PERT_1*0.5 + UY_PERT_2 ;
   vz = maxwellian_rand(vthe) - JZ_PERT_1*0.5 - JZ_PERT_2*0.5 ;  // add drift
   v2 = vx*vx + vy*vy + vz*vz;

   //  Need to make sure that v < c
   //  If not - then pick a new random velocity from Maxwellian
   ...
   //  Now convert this pack to a relativistic Momentum
   ...
   uz = gamma*vz;
    ...
    //uz = maxwellian_rand(vthi) + 0.05 ;

    //  Same procedure as above 

   vx = maxwellian_rand(vthe) + UX_PERT_1 + JX_PERT_2*0.5 ;
   vy = maxwellian_rand(vthe) + JY_PERT_1*0.5 + UY_PERT_2 ;
   vz = maxwellian_rand(vthe) + JZ_PERT_1*0.5 + JZ_PERT_2*0.5 ;
   v2 = vx*vx + vy*vy + vz*vz;
  ...
  sim_log( "Finished loading particles" );
	...
	*/
	...
	global->fdParams.output_variables( electric | magnetic | div_b_err );
...
begin_diagnostics {
   ...
   *------------------------------------------------------------------------*/
	...
	if(should_dump(Hhydro)) hydro_dump("ion", global->hHdParams);
        ...
        /*--------------------------------------------------------------------------
         * Direct Time-averaging of fields and moments using Vadim's Routine
         *------------------------------------------------------------------------*/
