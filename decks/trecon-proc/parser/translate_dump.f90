...
integer nsp,n,myid
integer (kind=4) itype,tindex,tindex0,tstart,ncpu,nc,i,dt,ipart
...
do nc = 0, ncpu-1
  ...
  write(fname,"(A9,I0,A17,I0)") "tracer/T.",tindex,"/electron_tracer.",nc
  ...
  if (check) then
    ...
    print*, ipart
    do i=1,ipart
      read(13) tag_part, buffer_part
      ek1 = sqrt(1.0+buffer_part(4)**2+buffer_part(5)**2+buffer_part(6)**2)-1.0
      ek1 = ek1/ethe
  ...
  write(fname,"(A9,I0,A12,I0)") "tracer/T.",tindex,"/ion_tracer.",nc
  inquire(file=trim(fname),exist=check)
  ekmax_i = 0.0
  if (check) then
    open(unit=13,file=trim(fname),status='unknown',form='binary')
    ...
    do i=1,ipart
      ...
      ek1 = sqrt(1.0+buffer_part(4)**2+buffer_part(5)**2+buffer_part(6)**2)-1.0
      ek1 = ek1/ethi
      if (ek1>ekmax_i) ekmax_i = ek1
      if (ek1 > ek1_i .and. ek1 < ek2_i) then
        write(12) tag_part,ek1
        iselect_i = iselect_i + 1
        if (iselect_i>npartmax) iselect_i = npartmax
        numpar_i(iselect_i) = tag_part
      endif
    enddo
    close(13)
    print*, fname, ekmax_i, iselect_i
  endif
enddo
...
if (write_select) then
  ...
  itotal = 0
  do nc = 0, ncpu-1
    write(fname,"(A9,I0,A12,I0)") "tracer/T.",tindex0,"/ion_tracer.",nc
    write(fname_select,"(A16,I0,A12,I0)") "tracer_select/T.",tindex0,"/ion_tracer.",nc
    ...
    if (check) then
      ...
      do i=1, itype
        read(10) tag_part,buffer_part
        do iselect=1,nselect
          if (tag_part==numpar_i(iselect)) then
            !print*, "got",tag_part, numpar_i(iselect), iselect
            ipart = ipart + 1
            itotal = itotal + 1
            tag_select(ipart) = tag_part
            buffer_select(:,ipart) = buffer_part(:)
