...
begin_initialization {
 ...
 double eps0 = 1;         // Permittivity of space
  ...
  int particle_select = 100;

  // Physics parameters

  double mi_me   = 1.0;    // Ion mass / electron mass
  double L_di    = 6.0/sqrt(mi_me);  // Sheet thickness / ion inertial length
  ...
  double taui    = 1600/(wpe_wce*mi_me);      // simulation wci's to run Fan: transformed to wpe
  ...
  double Lx  = 600.0/sqrt(mi_me)*di; // size of box in x dimension Fan: tranformed to c/wpe
  double Ly  = 300.0/sqrt(mi_me)*di;     // size of box in y dimension
  double Lz  = 300.0/sqrt(mi_me)*di; // size of box in z dimension
...
//LO
  ...
  repeat ( Ne/nproc() ) {
   ...
   //  Load electrons as drifting Maxwellian with velocity specified to be consistent with B field
    ...
    //ux = maxwellian_rand(vthe) + 0.5*VDX;
    //uy = maxwellian_rand(vthe) + 0.5*VDY;
    ...
    //ux = maxwellian_rand(vthi) - 0.5*VDX;
    //uy = maxwellian_rand(vthi) - 0.5*VDY;
...
//    uy = upe1;
    ...
    if (particle_tracing == 1){
     if (i%particle_select == 0){
      ...
      int tag = ((rank_int<<19) | (itp & 0x7ffff)); // 13 bits (8192) for rank and 19 bits (~520k)
	...
	if  (( step>0 && global->quota_check_interval>0 && (step&global->quota_check_interval)==0) || (global->write_end_restart) ) {
	   if ( (global->write_end_restart) ) {
	    ...
	    exit(0); // Exit or abort?
         ...
         if( mp_elapsed( grid->mp ) > global->quota_sec )   global->write_end_restart = 1;
	}
